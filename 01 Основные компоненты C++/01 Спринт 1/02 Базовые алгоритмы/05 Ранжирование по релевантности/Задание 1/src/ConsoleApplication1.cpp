...
vector<pair<int, int>> FindTopDocuments(
    ...
    const string& query) {
    vector<pair<int, int>> found_documents;
    for (auto [relevance, document_id] : FindAllDocuments(word_to_documents, stop_words, query)) {
        found_documents.push_back(make_pair(document_id, relevance));
    }
    int kol = found_documents.size();
    for (int i = MAX_RESULT_DOCUMENT_COUNT - 1; i < kol-1; i++)
        found_documents.pop_back();
    return found_documents;
}
