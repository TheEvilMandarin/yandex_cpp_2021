...
public:
	...
	void AddDocument(int document_id, const string& document) {
		...
		for (const string& word : words) {
			word_to_document_freqs_[word].insert(make_pair(document_id, (double)count(words.begin(), words.end(), word) / (double)words.size()));
		}
		document_count_++;
	}
...
private:
	...
	vector<Document> FindAllDocuments(const string& query) const {
		...
		for (const string& word : query_words) {
			...
			auto it = word_to_document_freqs_.at(word);
			double kol = it.size();
			double IDF = log((double)document_count_ / (double)kol);

			for (auto [document_id, document_freq] : word_to_document_freqs_.at(word)) {
				document_to_relevance[document_id] += IDF * document_freq;
			}
